// Runs all three imports (orders, weekly, monthly) in sequence, straight
// against the DB -- no HTTP call, no auth needed. Meant to be run on a
// schedule via the VPS's crontab, through the `migrate` Docker service
// (same one used for prisma migrate deploy):
//
//   crontab -e
...
// WEEKLY_SOURCE_REF / MONTHLY_SOURCE_REF are set to a live URL (e.g. a
// Google Sheets "publish to web" link) instead of a local file path --
// pointed at a local file, this just re-imports the same file that's
// already there. See .env.prod.example for how to set a live URL.
