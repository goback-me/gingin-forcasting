...
// (type is one of orders | weekly | monthly, defaults to orders)
//
...
// MONTHLY_SOURCE_REF point at a live URL instead of a local file -- see
// scripts/scheduled-import.ts for a version that runs all three directly
// (no HTTP round-trip, no auth needed) from a VPS crontab entry instead.
export async function POST(req: NextRequest) {
  const type = req.nextUrl.searchParams.get("type") || "orders";
  const result =
    type === "weekly" ? await importWeeklySales() : type === "monthly" ? await importMonthlySales() : await importOrders();
